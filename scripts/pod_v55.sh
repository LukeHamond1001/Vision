#!/usr/bin/env bash
# v5.5 CONTINUATION (A26): resume the just-ignited v5.4 circuit through the unseen corpus tail: attention + band latents + live imagination + full drive, past the emergence threshold. Debug-width model, dense
...
hb() {
  ...
  for f in HEARTBEAT.log train_tail.log eval_results.txt prep.log v55.pt.trace.jsonl; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-v55 2>/dev/null || true
}

hb "boot v55-continuation $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) ram $(free -g 2>/dev/null | awk '/Mem:/{print $2}')G"
...
# continuation seed: the v5.4 final checkpoint + its exact tokenizer
git fetch -q origin results-v54 --depth 1
for pp in aa ab ac; do
  git show FETCH_HEAD:v54_part_$pp > v54_part_$pp
done
cat v54_part_aa v54_part_ab v54_part_ac > v54.pt && rm -f v54_part_*
mkdir -p data/tokref
git show FETCH_HEAD:data/uc_v54_eval/tokenizer.json > data/tokref/tokenizer.json
if [ ! -s v54.pt ] || [ ! -s data/tokref/tokenizer.json ]; then
  hb "V54 ARTIFACT FETCH FAILED - aborting"
  runpodctl remove pod "$RUNPOD_POD_ID" || true
  sleep 120; exit 1
fi
hb "v54 checkpoint + tokenizer fetched ($(stat -c%s v54.pt) bytes)"
...
prepare("data/uc_v55", n_convos=1500000, seed=0, vocab=16384,
...
if [ "$PREP_RC" -ne 0 ] || [ ! -s data/uc_v55/tokens.bin ]; then
  hb "PREP FAILED (rc=$PREP_RC, tokens.bin $( [ -f data/uc_v55/tokens.bin ] && echo present || echo MISSING)) - see prep.log"
  runpodctl remove pod "$RUNPOD_POD_ID" || true
  sleep 120; exit 1
fi
rm -f data/ultrachat_raw.jsonl
hb "prep: train shard built ($(stat -c%s data/uc_v55/tokens.bin) bytes tokens.bin, raw deleted)"
if python - >> prep.log 2>&1 <<'EOF'
import os
os.environ["ULTRACHAT_JSONL"] = "data/ultrachat_heldout.jsonl"
from iga.lm_data_ultrachat import prepare
prepare("data/uc_v55_eval", n_convos=2500, seed=2, instrument_every=1,
        tokenizer_path="data/uc_v55/tokenizer.json")
EOF
then
...
( C=0; while true; do sleep 900; C=$((C+1)); \
    ...
    if [ $((C % 4)) -eq 0 ] && [ -f v55.pt ]; then \
      cp v55.pt /workspace/snap/s.pt && \
      ( cd /workspace/snap && rm -f snap_part_* && \
        split -b 25m s.pt snap_part_ && rm -f s.pt && \
        git add -A && git commit -qm "rolling snapshot" && \
        git push -qf "$PUSH" snap:results-v55-ckpt ) && \
      hb "rolling ckpt snapshot pushed"; \
    fi; done ) &
HBPID=$!
python -m iga.lm_train run --data data/uc_v55 --d 128 --lanes 32 \
  --chunk 512 --steps 28000 --talk tick --arch hybrid --device cuda \
  --resume v54.pt --offset 0.7909347 \
  --ckpt v55.pt > train.log 2>&1
TRAIN_RC=$?
kill $HBPID 2>/dev/null
tail -60 train.log > train_tail.log
hb "training complete (rc=$TRAIN_RC)"

if python -m iga.lm_eval --ckpt v55.pt --data data/uc_v55_eval \
...
split -b 25m v55.pt v55_part_
CKPT_OK=1
for f in $(ls v55_part_*); do
  git add -f "$f" && git commit -qm "ckpt piece: $f"
  PUSHED=0
  for i in 1 2 3 4; do
    if git push -f "$PUSH" results-v55 && \
       [ "$(git ls-remote "$PUSH" results-v55 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
      PUSHED=1; break
    fi
    sleep 20
  done
...
git add -f data/uc_v55_eval/tokens.bin data/uc_v55_eval/events.jsonl \
  data/uc_v55_eval/tokenizer.json train.log prep.log \
  v55.pt.trace.jsonl 2>/dev/null || true
