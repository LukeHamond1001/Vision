#!/usr/bin/env bash
# v5.2 CRAFTER-ON-LLM (A17): standard transformer + full drive layer. Debug-width model, dense
...
hb() {
  ...
  git push -qf "$PUSH" results-v52 2>/dev/null || true
}

hb "boot v52 $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
...
prepare("data/uc_v52", n_convos=150000, seed=0, vocab=16384,
        instrument_every=1)
EOF
...
from iga.lm_data_ultrachat import prepare
prepare("data/uc_v52_eval", n_convos=2500, seed=2, instrument_every=1,
        tokenizer_path="data/uc_v52/tokenizer.json")
EOF
...
( C=0; while true; do sleep 900; C=$((C+1)); \
    ...
    if [ $((C % 4)) -eq 0 ] && [ -f v52.pt ]; then \
      cp v52.pt /workspace/snap/s.pt && \
      ( cd /workspace/snap && rm -f snap_part_* && \
        split -b 25m s.pt snap_part_ && rm -f s.pt && \
        git add -A && git commit -qm "rolling snapshot" && \
        git push -qf "$PUSH" snap:results-v52-ckpt ) && \
      hb "rolling ckpt snapshot pushed"; \
    fi; done ) &
HBPID=$!
python -m iga.lm_train run --data data/uc_v52 --d 128 --lanes 32 \
  --chunk 512 --steps 10000 --talk tick --arch transformer --device cuda \
  --ckpt v52.pt > train.log 2>&1
TRAIN_RC=$?
kill $HBPID 2>/dev/null
tail -60 train.log > train_tail.log
hb "training complete (rc=$TRAIN_RC)"

if python -m iga.lm_eval --ckpt v52.pt --data data/uc_v52_eval \
    --d 128 --arch transformer --chunk 512 --lanes 4 --chunks 200 \
...
split -b 25m v52.pt v52_part_
BASE_SHA=$(git rev-parse HEAD)
CKPT_OK=1
for f in $(ls v52_part_*); do
  git add -f "$f" && git commit -qm "ckpt piece: $f"
  PUSHED=0
  for i in 1 2 3 4; do
    if git push -f "$PUSH" results-v52 && \
       [ "$(git ls-remote "$PUSH" results-v52 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
      PUSHED=1; break
    fi
    sleep 20
  done
...
git add -f data/uc_v52_eval/tokens.bin data/uc_v52_eval/events.jsonl \
  data/uc_v52_eval/tokenizer.json train.log prep.log 2>/dev/null || true
