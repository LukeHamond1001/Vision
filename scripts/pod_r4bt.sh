#!/usr/bin/env bash
# A52b R4b trainer: TOKEN-KEYED STORAGE. R1's exact config with one
...
W=/workspace/w-r4b
rm -rf "$W" /workspace/w-r4 && mkdir -p "$W" && cd "$W"
...
hb() {
  ...
  git push -qf "$PUSH" results-r4b 2>/dev/null || \
    { sleep 15; git push -qf "$PUSH" results-r4b 2>/dev/null; } || \
    { sleep 45; git push -qf "$PUSH" results-r4b 2>/dev/null; } || true
}
hb "boot r4b-DETACHED-RECON trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
...
CKPT_OK=0
if [ -f r4b.pt ]; then
  split -b 25m r4b.pt r4b_part_
  [ -f r4b.pt.best.pt ] && split -b 25m r4b.pt.best.pt r4bbest_part_
  CKPT_OK=1
  for f in $(ls r4b_part_* r4bbest_part_* 2>/dev/null); do
    git add -f "$f" && git commit -qm "ckpt piece: $f"
    PUSHED=0
    for i in 1 2 3 4; do
      if git push -f "$PUSH" results-r4b && \
         [ "$(git ls-remote "$PUSH" results-r4b | cut -f1)" = "$(git rev-parse HEAD)" ]; then
        PUSHED=1; break
      fi
      sleep 20
    done
...
git add -f train.log r4b.pt.trace.jsonl 2>/dev/null || true
git commit -qm "logs" 2>/dev/null || true
git push -qf "$PUSH" results-r4b 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f r4b.pt ]; then git reset --mixed "$BASE_SHA"; fi
