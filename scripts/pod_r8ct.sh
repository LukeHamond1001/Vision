...
git checkout -B results-r8c

hb() {
  echo "$(date -u '+%H:%M:%S') $1" >> HEARTBEAT.log
  for f in HEARTBEAT.log train_tail.log eval_results.txt \
           r8c.pt.trace.jsonl canary.log; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-r8c 2>/dev/null || \
    { sleep 15; git push -qf "$PUSH" results-r8c 2>/dev/null; } || \
    { sleep 45; git push -qf "$PUSH" results-r8c 2>/dev/null; } || true
}
hb "boot r8c-AUX01 trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
...
CKPT_OK=0
if [ -f r8c.pt ]; then
  split -b 25m r8c.pt r8c_part_
  [ -f r8c.pt.best.pt ] && split -b 25m r8c.pt.best.pt r8cbest_part_
  CKPT_OK=1
  for f in $(ls r8c_part_* r8cbest_part_* 2>/dev/null); do
    git add -f "$f" && git commit -qm "ckpt piece: $f"
    PUSHED=0
    for i in 1 2 3 4; do
      if git push -f "$PUSH" results-r8c && \
         [ "$(git ls-remote "$PUSH" results-r8c | cut -f1)" = "$(git rev-parse HEAD)" ]; then
        PUSHED=1; break
      fi
      sleep 20
    done
...
git add -f train.log r8c.pt.trace.jsonl 2>/dev/null || true
git commit -qm "logs" 2>/dev/null || true
git push -qf "$PUSH" results-r8c 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f r8c.pt ]; then git reset --mixed "$BASE_SHA"; fi
