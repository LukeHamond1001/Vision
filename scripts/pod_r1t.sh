...
hb() {
  ...
  for f in HEARTBEAT.log train_tail.log eval_results.txt r1.pt.trace.jsonl; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-r1 2>/dev/null || true
}
hb "boot r1-GATED-READS trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
...
python -m iga.lm_train run --data /workspace/rmix/mix_r1 --d 256 --lanes 16 \
  ...
  --store matrix --xl off --lr 1.5e-4 --gate-init -2 \
...
if [ -f r1.pt ]; then
  ...
  [ -f r1.pt.best.pt ] && split -b 25m r1.pt.best.pt r1best_part_
  CKPT_OK=1
...
git add -f data/mix_r1_eval/tokens.bin data/mix_r1_eval/events.jsonl \
  data/mix_r1_eval/tokenizer.json train.log r1.pt.trace.jsonl 2>/dev/null || true
git commit -qm "eval shard + logs" 2>/dev/null || true
git push -qf "$PUSH" results-r1 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f r1.pt ]; then git reset --mixed "$BASE_SHA"; fi
