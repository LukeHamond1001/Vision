...
hb() {
  ...
  for f in HEARTBEAT.log train_tail.log eval_results.txt prep.log v63.pt.trace.jsonl; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-v63 2>/dev/null || true
}

hb "boot v63-CAPPED-GATES-BESTCKPT $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) ram $(free -g 2>/dev/null | awk '/Mem:/{print $2}')G"
...
prepare("data/uc_v63", n_convos=1500000, seed=0, vocab=16384,
...
if [ "$PREP_RC" -ne 0 ] || [ ! -s data/uc_v63/tokens.bin ]; then
  hb "PREP FAILED (rc=$PREP_RC, tokens.bin $( [ -f data/uc_v63/tokens.bin ] && echo present || echo MISSING)) - see prep.log"
  runpodctl remove pod "$RUNPOD_POD_ID" || true
  sleep 120; exit 1
fi
rm -f data/ultrachat_raw.jsonl
hb "prep: train shard built ($(stat -c%s data/uc_v63/tokens.bin) bytes tokens.bin, raw deleted)"
if python - >> prep.log 2>&1 <<'EOF'
import os
os.environ["ULTRACHAT_JSONL"] = "data/ultrachat_heldout.jsonl"
from iga.lm_data_ultrachat import prepare
prepare("data/uc_v63_eval", n_convos=2500, seed=2, instrument_every=1,
        tokenizer_path="data/uc_v63/tokenizer.json")
EOF
then
...
( C=0; while true; do sleep 900; C=$((C+1)); \
    ...
    if [ $((C % 4)) -eq 0 ] && [ -f v63.pt ]; then \
      cp v63.pt /workspace/snap/s.pt && \
      ( cd /workspace/snap && rm -f snap_part_* && \
        split -b 25m s.pt snap_part_ && rm -f s.pt && \
        git add -A && git commit -qm "rolling snapshot" && \
        git push -qf "$PUSH" snap:results-v63-ckpt ) && \
      hb "rolling ckpt snapshot pushed"; \
    fi; done ) &
HBPID=$!
python -m iga.lm_train run --data data/uc_v63 --d 128 --lanes 32 \
  --chunk 512 --steps 135000 --talk tick --arch hybrid --device cuda \
  --store matrix --xl off --gate-mode position --eval-data data/uc_v63_eval \
  --ckpt v63.pt > train.log 2>&1
TRAIN_RC=$?
kill $HBPID 2>/dev/null
tail -60 train.log > train_tail.log
hb "training complete (rc=$TRAIN_RC)"

if python -m iga.lm_eval --ckpt v63.pt --data data/uc_v63_eval \
...
if [ -f v63.pt.best.pt ]; then
  echo "===== BEST-HOLDOUT CKPT =====" >> eval_results.txt
  python -m iga.lm_eval --ckpt v63.pt.best.pt --data data/uc_v63_eval \
    --d 128 --arch hybrid --store matrix --xl off --gate-mode position --chunk 512 --lanes 4 --chunks 200 \
...
split -b 25m v63.pt v63_part_
if [ -f v63.pt.best.pt ]; then
  split -b 25m v63.pt.best.pt v63best_part_
fi
CKPT_OK=1
for f in $(ls v63_part_* v63best_part_* 2>/dev/null); do
  git add -f "$f" && git commit -qm "ckpt piece: $f"
  PUSHED=0
  for i in 1 2 3 4; do
    if git push -f "$PUSH" results-v63 && \
       [ "$(git ls-remote "$PUSH" results-v63 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
      PUSHED=1; break
    fi
    sleep 20
  done
...
git add -f data/uc_v63_eval/tokens.bin data/uc_v63_eval/events.jsonl \
  data/uc_v63_eval/tokenizer.json train.log prep.log \
  v63.pt.trace.jsonl 2>/dev/null || true
