...
hb() {
  ...
  for f in HEARTBEAT.log train_tail.log eval_results.txt prep.log r1.pt.trace.jsonl; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-r1 2>/dev/null || true
}

hb "boot r1-GATED-READS-ON-CODE $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) ram $(free -g 2>/dev/null | awk '/Mem:/{print $2}')G disk $(df -BG /workspace | awk 'NR==2{print $4}')"
...
sources = [source_code_jsonl("data/code_texts.jsonl"),
           ...
           source_chat(120000)]
prepare_mix("data/mix_r1", sources, budget_tokens=1_200_000_000,
...
sources_ev = [source_code_jsonl("data/code_eval.jsonl"),
              ...
              source_chat(7000)]
prepare_mix("data/mix_r1_eval", sources_ev, budget_tokens=7_000_000,
            seed=99, vocab=32768,
            tokenizer_path="data/mix_r1/tokenizer.json",
            mine_ids=True)
PREPEOF
PREP_PID=$!
...
if [ "$PREP_RC" -ne 0 ] || [ ! -s data/mix_r1/tokens.bin ]; then
  hb "PREP FAILED (rc=$PREP_RC) - see prep.log"
  runpodctl remove pod "$RUNPOD_POD_ID" || true
  sleep 120; exit 1
fi
rm -f data/code_texts.jsonl data/ultrachat_raw.jsonl data/mixsrc/wiki*.parquet
hb "prep: mix shard built ($(stat -c%s data/mix_r1/tokens.bin) bytes, raw deleted)"
...
( C=0; while true; do sleep 900; C=$((C+1)); \
    ...
    if [ $((C % 8)) -eq 0 ] && [ -f r1.pt ]; then \
      cp r1.pt /workspace/snap/s.pt && \
      ( cd /workspace/snap && rm -f snap_part_* && \
        split -b 25m s.pt snap_part_ && rm -f s.pt && \
        git add -A && git commit -qm "rolling snapshot" && \
        git push -qf "$PUSH" snap:results-r1-ckpt ) && \
      hb "rolling ckpt snapshot pushed"; \
    fi; done ) &
HBPID=$!
python -m iga.lm_train run --data data/mix_r1 --d 256 --lanes 16 \
  --chunk 1024 --steps 75000 --talk tick --arch hybrid --device cuda \
  --store matrix --xl off --lr 1.5e-4 --gate-init -2 --eval-data data/mix_r1_eval \
  --ckpt r1.pt > train.log 2>&1
TRAIN_RC=$?
kill $HBPID 2>/dev/null
tail -60 train.log > train_tail.log
hb "training complete (rc=$TRAIN_RC)"

for CK in r1.pt r1.pt.best.pt; do
  if [ -f "$CK" ]; then
    echo "===== EVAL $CK =====" >> eval_results.txt
    python -m iga.lm_eval --ckpt "$CK" --data data/mix_r1_eval \
...
split -b 25m r1.pt r1_part_
if [ -f r1.pt.best.pt ]; then
  split -b 25m r1.pt.best.pt r1best_part_
fi
CKPT_OK=1
for f in $(ls r1_part_* r1best_part_* 2>/dev/null); do
  git add -f "$f" && git commit -qm "ckpt piece: $f"
  PUSHED=0
  for i in 1 2 3 4; do
    if git push -f "$PUSH" results-r1 && \
       [ "$(git ls-remote "$PUSH" results-r1 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
      PUSHED=1; break
    fi
    sleep 20
  done
...
git add -f data/mix_r1_eval/tokens.bin data/mix_r1_eval/events.jsonl \
  data/mix_r1_eval/tokenizer.json train.log prep.log \
  r1.pt.trace.jsonl 2>/dev/null || true
