#!/usr/bin/env bash
# A59 v9.2: the closing scale run — R8b's recipe at v9's certified
# ops config: norm_mix + aux-head 0.2 (the MEASURED dose: the only
# protection that flipped the starvation force's sign — r8b trunk
# 5.9% stronger standalone than no-aux at equal duration) on d=512
# T=2048 lanes 6, 488k steps, cosine, kill rule armed locally.
set -uo pipefail
...
import torch
try:
    print(torch.load("v92.pt", map_location="cpu",
...
git checkout -B results-v92

hb() {
  echo "$(date -u '+%H:%M:%S') $1" >> HEARTBEAT.log
  for f in HEARTBEAT.log train_tail.log eval_results.txt \
           v92.pt.trace.jsonl canary.log smoke_tail.log; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-v92 2>/dev/null || \
    { sleep 15; git push -qf "$PUSH" results-v92 2>/dev/null; } || \
    { sleep 45; git push -qf "$PUSH" results-v92 2>/dev/null; } || true
}
hb "boot v92-AUXHEAD trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) resume='${RESUME}'"
...
python -m iga.lm_train run --data /workspace/rmix/mix_v9 --d 512 \
  ...
  --keyed logit --norm-mix --aux-trunk 0.2 --ckpt smoke.pt > smoke.log 2>&1
SMOKE_RC=$?
...
( LAST_STEP=0; BEAT=0
  while true; do
    ...
    # <=2h, not the run (atomic_save keeps v92.pt always-complete)
    if [ $((BEAT % 4)) -eq 0 ] && [ -f v92.pt ]; then
      ( rm -rf /workspace/w-v92/snap && mkdir -p /workspace/w-v92/snap &&
        cp v92.pt /workspace/w-v92/snap/ && cd /workspace/w-v92/snap &&
        split -b 25m v92.pt v92roll_ && rm v92.pt &&
        git init -q . && git checkout -q -b results-v92-ckpt &&
        git add . &&
        git -c user.email=pod@iga -c user.name=pod commit -qm "rolling beat $BEAT step $CUR" &&
        git push -qf "$PUSH" results-v92-ckpt ) >/dev/null 2>&1 \
...
CKPT_OK=0
if [ -f v92.pt ]; then
  split -b 25m v92.pt v92_part_
  [ -f v92.pt.best.pt ] && split -b 25m v92.pt.best.pt v92best_part_
  CKPT_OK=1
  for f in $(ls v92_part_* v92best_part_* 2>/dev/null); do
    git add -f "$f" && git commit -qm "ckpt piece: $f"
    PUSHED=0
    for i in 1 2 3 4; do
      if git push -f "$PUSH" results-v92 && \
         [ "$(git ls-remote "$PUSH" results-v92 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
        PUSHED=1; break
      fi
      sleep 20
    done
...
git add -f train.log v92.pt.trace.jsonl 2>/dev/null || true
git commit -qm "logs" 2>/dev/null || true
git push -qf "$PUSH" results-v92 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f v92.pt ]; then git reset --mixed "$BASE_SHA"; fi
