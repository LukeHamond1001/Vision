#!/usr/bin/env bash
# A58b R8b trainer: SEPARATE AUX HEAD. R8's exact config, one
# variable: the pay-the-trunk aux CE now reads its own
# Linear(d,V) probe — R8 proved the shared head compromises
# both objectives (full CE +9%); trunk BLOCKS still earn the
# full anti-starvation gradient. Volume read-only. 24GB floor.
set -uo pipefail
...
git checkout -B results-r8b

hb() {
  echo "$(date -u '+%H:%M:%S') $1" >> HEARTBEAT.log
  for f in HEARTBEAT.log train_tail.log eval_results.txt \
           r8b.pt.trace.jsonl canary.log; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-r8b 2>/dev/null || \
    { sleep 15; git push -qf "$PUSH" results-r8b 2>/dev/null; } || \
    { sleep 45; git push -qf "$PUSH" results-r8b 2>/dev/null; } || true
}
hb "boot r8b-AUXHEAD trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
...
CKPT_OK=0
if [ -f r8b.pt ]; then
  split -b 25m r8b.pt r8b_part_
  [ -f r8b.pt.best.pt ] && split -b 25m r8b.pt.best.pt r8bbest_part_
  CKPT_OK=1
  for f in $(ls r8b_part_* r8bbest_part_* 2>/dev/null); do
    git add -f "$f" && git commit -qm "ckpt piece: $f"
    PUSHED=0
    for i in 1 2 3 4; do
      if git push -f "$PUSH" results-r8b && \
         [ "$(git ls-remote "$PUSH" results-r8b | cut -f1)" = "$(git rev-parse HEAD)" ]; then
        PUSHED=1; break
      fi
      sleep 20
    done
...
git add -f train.log r8b.pt.trace.jsonl 2>/dev/null || true
git commit -qm "logs" 2>/dev/null || true
git push -qf "$PUSH" results-r8b 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f r8b.pt ]; then git reset --mixed "$BASE_SHA"; fi
