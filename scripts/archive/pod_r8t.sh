#!/usr/bin/env bash
# A58 R8 trainer: PAY THE TRUNK. R6's exact config with one
# variable (--aux-trunk 0.2): auxiliary CE on the pre-bonus
# logits — the trunk earns gradient on every position no matter
# how much the store covers (A57c: geometry, volume, dose all
# exonerated; the bleed is gradient starvation, so feed it).
# Volume read-only (nothing else may be touched).
set -uo pipefail
...
git checkout -B results-r8

hb() {
  echo "$(date -u '+%H:%M:%S') $1" >> HEARTBEAT.log
  for f in HEARTBEAT.log train_tail.log eval_results.txt \
           r8.pt.trace.jsonl canary.log; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-r8 2>/dev/null || \
    { sleep 15; git push -qf "$PUSH" results-r8 2>/dev/null; } || \
    { sleep 45; git push -qf "$PUSH" results-r8 2>/dev/null; } || true
}
hb "boot r8-AUXTRUNK trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1)"
...
CKPT_OK=0
if [ -f r8.pt ]; then
  split -b 25m r8.pt r8_part_
  [ -f r8.pt.best.pt ] && split -b 25m r8.pt.best.pt r8best_part_
  CKPT_OK=1
  for f in $(ls r8_part_* r8best_part_* 2>/dev/null); do
    git add -f "$f" && git commit -qm "ckpt piece: $f"
    PUSHED=0
    for i in 1 2 3 4; do
      if git push -f "$PUSH" results-r8 && \
         [ "$(git ls-remote "$PUSH" results-r8 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
        PUSHED=1; break
      fi
      sleep 20
    done
...
git add -f train.log r8.pt.trace.jsonl 2>/dev/null || true
git commit -qm "logs" 2>/dev/null || true
git push -qf "$PUSH" results-r8 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f r8.pt ]; then git reset --mixed "$BASE_SHA"; fi
