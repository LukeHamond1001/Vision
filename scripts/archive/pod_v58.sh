...
hb() {
  ...
  for f in HEARTBEAT.log train_tail.log eval_results.txt prep.log v58.pt.trace.jsonl; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-v58 2>/dev/null || true
}

hb "boot v58-decomp $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) ram $(free -g 2>/dev/null | awk '/Mem:/{print $2}')G"
...
prepare("data/uc_v58", n_convos=1500000, seed=0, vocab=16384,
...
if [ "$PREP_RC" -ne 0 ] || [ ! -s data/uc_v58/tokens.bin ]; then
  hb "PREP FAILED (rc=$PREP_RC, tokens.bin $( [ -f data/uc_v58/tokens.bin ] && echo present || echo MISSING)) - see prep.log"
  runpodctl remove pod "$RUNPOD_POD_ID" || true
  sleep 120; exit 1
fi
rm -f data/ultrachat_raw.jsonl
hb "prep: train shard built ($(stat -c%s data/uc_v58/tokens.bin) bytes tokens.bin, raw deleted)"
if python - >> prep.log 2>&1 <<'EOF'
import os
os.environ["ULTRACHAT_JSONL"] = "data/ultrachat_heldout.jsonl"
from iga.lm_data_ultrachat import prepare
prepare("data/uc_v58_eval", n_convos=2500, seed=2, instrument_every=1,
        tokenizer_path="data/uc_v58/tokenizer.json")
EOF
then
...
split -b 25m v58.pt v58_part_
CKPT_OK=1
for f in $(ls v58_part_*); do
  git add -f "$f" && git commit -qm "ckpt piece: $f"
  PUSHED=0
  for i in 1 2 3 4; do
    if git push -f "$PUSH" results-v58 && \
       [ "$(git ls-remote "$PUSH" results-v58 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
      PUSHED=1; break
    fi
    sleep 20
  done
...
git add -f data/uc_v58_eval/tokens.bin data/uc_v58_eval/events.jsonl \
  data/uc_v58_eval/tokenizer.json train.log prep.log \
  v58.pt.trace.jsonl 2>/dev/null || true
