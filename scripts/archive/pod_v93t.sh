#!/usr/bin/env bash
# A60 v9.3: THE DENSITY FIX AT SCALE (practice-like-we-play).
# v9.2's recipe + ONE variable: --hold-cap 1 throttles drive
# minting to the measured-safe r-scale rate (~0.85/step vs
# ~13/step uncapped — A59b: onset invariant to every store-side
# knob; density is the last suspect standing). Kill zone ~70-90k
# is the verdict instrument; surviving it, the run IS production.
set -uo pipefail
...
import torch
try:
    print(torch.load("v93.pt", map_location="cpu",
...
git checkout -B results-v93

hb() {
  echo "$(date -u '+%H:%M:%S') $1" >> HEARTBEAT.log
  for f in HEARTBEAT.log train_tail.log eval_results.txt \
           v93.pt.trace.jsonl canary.log smoke_tail.log; do
    git add -f "$f" 2>/dev/null || true
  done
  git commit -qm "hb: $1" 2>/dev/null || true
  git push -qf "$PUSH" results-v93 2>/dev/null || \
    { sleep 15; git push -qf "$PUSH" results-v93 2>/dev/null; } || \
    { sleep 45; git push -qf "$PUSH" results-v93 2>/dev/null; } || true
}
hb "boot v93-HOLDCAP trainer $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1) resume='${RESUME}'"
...
python -m iga.lm_train run --data /workspace/rmix/mix_v9 --d 512 \
  ...
  --keyed logit --norm-mix --aux-trunk 0.2 --hold-cap 1 --ckpt smoke.pt > smoke.log 2>&1
SMOKE_RC=$?
...
( LAST_STEP=0; BEAT=0
  while true; do
    ...
    # <=2h, not the run (atomic_save keeps v93.pt always-complete)
    if [ $((BEAT % 4)) -eq 0 ] && [ -f v93.pt ]; then
      ( rm -rf /workspace/w-v93/snap && mkdir -p /workspace/w-v93/snap &&
        cp v93.pt /workspace/w-v93/snap/ && cd /workspace/w-v93/snap &&
        split -b 25m v93.pt v93roll_ && rm v93.pt &&
        git init -q . && git checkout -q -b results-v93-ckpt &&
        git add . &&
        git -c user.email=pod@iga -c user.name=pod commit -qm "rolling beat $BEAT step $CUR" &&
        git push -qf "$PUSH" results-v93-ckpt ) >/dev/null 2>&1 \
...
CKPT_OK=0
if [ -f v93.pt ]; then
  split -b 25m v93.pt v93_part_
  [ -f v93.pt.best.pt ] && split -b 25m v93.pt.best.pt v93best_part_
  CKPT_OK=1
  for f in $(ls v93_part_* v93best_part_* 2>/dev/null); do
    git add -f "$f" && git commit -qm "ckpt piece: $f"
    PUSHED=0
    for i in 1 2 3 4; do
      if git push -f "$PUSH" results-v93 && \
         [ "$(git ls-remote "$PUSH" results-v93 | cut -f1)" = "$(git rev-parse HEAD)" ]; then
        PUSHED=1; break
      fi
      sleep 20
    done
...
git add -f train.log v93.pt.trace.jsonl 2>/dev/null || true
git commit -qm "logs" 2>/dev/null || true
git push -qf "$PUSH" results-v93 2>/dev/null || true
if [ "$CKPT_OK" != "1" ] && [ -f v93.pt ]; then git reset --mixed "$BASE_SHA"; fi
