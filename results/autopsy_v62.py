...
SHARD = os.path.join(S, "uc_v62_eval")
...
ck = torch.load(os.path.join(S, "v62_final.pt"), map_location="cpu",
                weights_only=False)
m = HybridLM(tok.get_vocab_size(), d=128, max_T=512, store="matrix", gate_mode="position",
             use_xl=False)
m.load_state_dict(ck["model"])
m.eval()
print("step:", ck.get("step"))
print("bias-gates:", {k: round(float(torch.sigmoid(l.bias)), 4)
                      for k, l in m.read_gate_pos.items()},
      "| weight norms:", {k: round(float(l.weight.norm()), 3)
                          for k, l in m.read_gate_pos.items()})
